...
- **`prep_powerflex_monitoring.sh`** - Primary configuration script that:
  ...
  - Validates Gen1 cluster compatibility (mirroring data layouts only)
...
- **`configure_gw_rest_ip.sh`** - Gateway configuration helper that:
  ...
  - Validates cluster is Gen1 with Mirroring (FG/MG) storage pools
...
- PowerFlex Gen1 cluster with MG/FG storage pools
...
- Gen2 (Erasure Coding) clusters are not supported - use the Gen2 toolkit instead
