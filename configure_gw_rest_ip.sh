...
function checkgw_gen1 {
    if [ -z "$access_token" ]; then
        echo -e " *** WARNING: Could not obtain access token. Skipping Gen1 cluster validation.\n"
        return
    fi
...
try:
    ...
    if not sps:
        ...
        for sp in sps:
            ...
            parts.append(n + " (" + d + ")")
        names = ", ".join(parts)
        if types.issubset({"FineGranularity", "MediumGranularity"}):
            print("OK:" + names)
        elif types & {"ErasureCoding"}:
            print("GEN2:" + names)
        else:
            print("UNKNOWN:" + names)
...
' 2>/dev/null)
    ...
    case "$gen_check" in
        OK:*)
            echo -e " *** Gen1 (Mirroring) cluster confirmed. All storage pools use Fine/Medium Granularity."
            echo -e "     ${gen_check#OK:}\n"
            ;;
        GEN2:*)
            echo -e " *** ERROR: This toolkit requires a PowerFlex Gen1 cluster with Mirroring."
            echo -e "     Storage pools found: ${gen_check#GEN2:}"
            echo -e "     Erasure Coding storage pools indicate a Gen2 cluster.\n"
            echo -e "     There is a monitoring toolkit available for PowerFlex Gen2 clusters.\n"
            exit 1
            ;;
        NO_SPS)
            echo -e " *** WARNING: No storage pools found on the cluster. Cannot verify Gen1 status.\n"
            ;;
        UNKNOWN:*)
            echo -e " *** ERROR: This toolkit requires a PowerFlex Gen1 cluster with Mirroring."
            echo -e "     Storage pools found: ${gen_check#UNKNOWN:}"
            echo -e "     Unrecognized storage pool layout type(s) detected. Cannot confirm Gen1 status.\n"
            exit 1
            ;;
        *)
